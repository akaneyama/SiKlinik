<x-app-layout>
    <div class="max-w-4xl mx-auto pb-12">
        ...
        <form method="POST" action="{{ route('products.store') }}" enctype="multipart/form-data"> {{-- TAMBAHKAN INI --}}
            @csrf
            <div class="bg-white p-8 rounded-lg shadow-lg" x-data="{ productType: '{{ old('jenis_produk', 'OBAT') }}' }">
                ...
                <div x-show="productType === 'OBAT'" x-transition class="p-4 mb-6 bg-green-50 border border-green-200 rounded-lg space-y-4">
                    ...
                    <div>
                        ...
                        <x-text-input id="nama_generik" name="nama_generik" type="text" class="mt-1 block w-full" :value="old('nama_generik')" />
 ...
 <div x-data="dropdownSearch()" class="relative w-full">
    ...
    <x-input-error :messages="$errors->get('bentuk_sediaan')" class="mt-2" />
                        ...
                        <div>
                            ...
                            <x-text-input id="batch" name="batch" type="text" class="mt-1 block w-full" :value="old('batch')" />
                            <x-input-error :messages="$errors->get('batch')" class="mt-2" />
                        </div>
                    </div>
                    <div>
                        <x-input-label for="tgl_kadaluarsa" value="Tanggal Kadaluarsa" />
                        <x-text-input id="tgl_kadaluarsa" name="tgl_kadaluarsa" type="date" class="mt-1 block w-full" :value="old('tgl_kadaluarsa')" />
                ...
                <div x-show="productType === 'SUPLAI'" x-transition class="p-4 mb-6 bg-purple-50 border border-purple-200 rounded-lg space-y-4">
                    ...
                    <div>
                        ...
                        <x-text-input id="no_seri" name="no_seri" type="text" class="mt-1 block w-full" :value="old('no_seri')" />
                        <x-input-error :messages="$errors->get('no_seri')" class="mt-2" />
                    </div>
                    <div>
                        <x-input-label for="tgl_akhir_garansi" value="Tanggal Akhir Garansi" />
                        <x-text-input id="tgl_akhir_garansi" name="tgl_akhir_garansi" type="date" class="mt-1 block w-full" :value="old('tgl_akhir_garansi')" />
                ...
                <div class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                         ...
                         <div>
                            ...
                            <x-text-input id="sku_barcode" name="sku_barcode" type="text" class="mt-1 block w-full" :value="old('sku_barcode')" required />
                            <x-input-error :messages="$errors->get('sku_barcode')" class="mt-2" />
                        </div>
                    </div>
                    <div>
                        <x-input-label for="gambar_produk" value="Gambar Produk (Opsional)" /> {{-- Input Gambar --}}
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                         <div>
                            ...
                            <x-text-input id="pabrikan" name="pabrikan" type="text" class="mt-1 block w-full" :value="old('pabrikan')" />
                            <x-input-error :messages="$errors->get('pabrikan')" class="mt-2" />
                        </div>
                         {{-- <div>
                            <x-input-label for="kategori" value="Kategori" />
                            <x-text-input id="kategori" name="kategori" type="text" class="mt-1 block w-full" :value="old('kategori')" />
                            <x-input-error :messages="$errors->get('kategori')" class="mt-2" />
                        </div> --}}
                        {{-- === INPUT KATEGORI (MULTI-SELECT) === --}}
                    <div x-data="multiselect()">
                        ...
                        <div class="relative mt-1">
                            <div class="w-full flex flex-wrap gap-2 p-2 border border-gray-300 rounded-md shadow-sm bg-white">
                                {{-- Menampilkan kategori yang sudah dipilih sebagai "badge" --}}
                                <template x-for="cat in selected" :key="cat">
                            ...
                            {{-- Daftar dropdown kategori yang bisa dipilih --}}
                            <div x-show="open" x-transition class="absolute z-10 mt-1 w-full bg-white border border-gray-300 rounded-md shadow-lg max-h-60 overflow-auto">
                                ...
                                <div x-show="filtered.length === 0" class="px-3 py-2 text-gray-500">
                                    Tidak ada hasil.
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            ...
                            <select name="kontrol_stok" id="kontrol_stok" class="block mt-1 w-full border-gray-300 rounded-md shadow-sm">
                                <option value="1" {{ old('kontrol_stok') == '1' ? 'selected' : '' }}>Aktif</option>
                                <option value="0" {{ old('kontrol_stok') == '0' ? 'selected' : '' }}>Tidak Aktif</option>
                            </select>
                            <x-input-error :messages="$errors->get('kontrol_stok')" class="mt-2" />
                        </div>
                        <div>
                            <x-input-label for="stok_awal" value="Stok Awal" />
                            <x-text-input id="stok_awal" name="stok_awal" type="number" step="any" class="mt-1 block w-full" :value="old('stok_awal', 0)" required />
                            <x-input-error :messages="$errors->get('stok_awal')" class="mt-2" />
                        </div>
                        <div class="grid grid-cols-2 gap-2">
                            <div>
                                <x-input-label for="min_stok" value="Min. Stok" />
                                <x-text-input id="min_stok" name="min_stok" type="number" step="any" class="mt-1 block w-full" :value="old('min_stok', 0)" required />
                                <x-input-error :messages="$errors->get('min_stok')" class="mt-2" />
                            </div>
                            <div>
                                <x-input-label for="max_stok" value="Max. Stok" />
                                <x-text-input id="max_stok" name="max_stok" type="number" step="any" class="mt-1 block w-full" :value="old('max_stok', 0)" required />
                ...
                <div class="space-y-4 mt-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        <div>
                            ...
                            <x-text-input id="harga_modal" name="harga_modal" type="number" step="any" class="mt-1 block w-full" :value="old('harga_modal')" required />
                        ...
                        <div>
                            ...
                            <x-text-input id="harga_modal_diskon" name="harga_modal_diskon" type="number" step="any" class="mt-1 block w-full" :value="old('harga_modal_diskon')" />
                            <x-input-error :messages="$errors->get('harga_modal_diskon')" class="mt-2" />
                        </div>
                        <div>
                            <x-input-label for="markup" value="Markup (%)" />
                            <x-text-input id="markup" name="markup" type="number" step="any" class="mt-1 block w-full" :value="old('markup')" />
                            <x-input-error :messages="$errors->get('markup')" class="mt-2" />
                        </div>
                        <div>
                            <x-input-label for="harga_jual" value="Harga Jual" />
                            <x-text-input id="harga_jual" name="harga_jual" type="number" step="any" class="mt-1 block w-full" :value="old('harga_jual')" required />
                            <x-input-error :messages="$errors->get('harga_jual')" class="mt-2" />
                        </div>
                        <div>
                            <x-input-label for="harga_spesial" value="Harga Spesial (Opsional)" />
                            <x-text-input id="harga_spesial" name="harga_spesial" type="number" step="any" class="mt-1 block w-full" :value="old('harga_spesial')" />
                        ...
                        <div>
                            ...
                            <select name="cakupan_harga" id="cakupan_harga" class="block mt-1 w-full border-gray-300 rounded-md shadow-sm">
                                <option value="Lokal" {{ old('cakupan_harga') == 'Lokal' ? 'selected' : '' }}>Lokal</option>
                                <option value="Global" {{ old('cakupan_harga') == 'Global' ? 'selected' : '' }}>Global</option>
                            </select>
                ...
                <div class="space-y-4 mt-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div>
                            ...
                            <x-text-input id="pembagi_konversi" name="pembagi_konversi" type="number" step="any" class="mt-1 block w-full" :value="old('pembagi_konversi', 1)" />
