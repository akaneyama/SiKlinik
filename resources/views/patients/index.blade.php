<x-app-layout>
    <div class="max-w-7xl mx-auto">
        <div class="flex justify-between items-center mb-6">
            ...
            <a href="{{ route('patients.create') }}" class="flex items-center px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700">
                Tambah Pasien
            </a>
        </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Keanggotaan</th>
                            ...
                            @forelse ($patients as $patient)
                                <tr class="hover:bg-gray-50">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        <div class="text-sm text-gray-500">{{ $patient->no_identitas }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        <div>{{ $patient->nomor_telp }}</div>
                                        <div>{{ $patient->email }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $patient->kota ?? 'N/A' }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $patient->membership?->jenis_keanggotaan ?? 'Umum' }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div class="flex items-center justify-end space-x-3">
                                            <a href="{{ route('patients.edit', $patient->id_pasien) }}" class="p-2 bg-yellow-400 rounded-full text-white hover:bg-yellow-500" title="Edit">
                                            ...
                                            <form action="{{ route('patients.destroy', $patient->id_pasien) }}" method="POST"  class="inline">
                                                @csrf
                                                @method('DELETE')
                                                ...
                                                <button type="button"
                                                class="p-2 bg-red-500 rounded-full text-white hover:bg-red-600" title="Hapus"
                                                onclick="openModal({{ $patient->id_pasien }})">
                                                <svg class="h-4 w-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                                            </button>
                                                <div id="deleteModal-{{ $patient->id_pasien  }}"
                                                class="hidden fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
                                                <div class="bg-white p-6 rounded-lg shadow-lg">
                                                    <h2 class="text-lg text-center font-semibold mb-2">Konfirmasi Hapus</h2>
                                                    <p class="mb-4 text-center">Apakah Anda yakin ingin menghapus pasien ini?</p>
                                                    <div class="flex justify-center gap-6">
                                                        <button type="button"
                                                                onclick="closeModal({{ $patient->id_pasien  }})"
                                                                class="px-4 py-2 bg-gray-300 rounded-lg hover:bg-gray-400">
                                                            Batal
                                                        </button>
                                                        <form action="{{ route('patients.destroy', $patient->id_pasien ) }}" method="POST">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button type="submit"
